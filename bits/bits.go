...
// and output keys for those chunks. Pushing to remote store happens at a later time (pre-push hook)
// hook
//
//                                         														 /-> FSIndex<Index>.Add(k) -> KeyEncoding.Encode(k, STDOUT)
//                 /--> Hash.Hash(chunk) --> Crypto.Encrypt(k, chunk) -|
...
// locally, attempt to fetch it from a remote and place it locally, then decrypt and combine them into the original
// file.
//
//                           /-> ...       																																																																																								...  ---\
